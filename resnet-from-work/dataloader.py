...
class PerImageStandardization(Preprocessor):
    def __init__(self):
        pass
    ...
    def apply_preprocessing(self, images):
        ...
        for image in images:
            ...
            image_std = image.std()
            adjusted_std = max(image_std, 1 / np.sqrt(reduce(lambda x, y: x * y, image.shape)))
            new_images.append((image - image_mean) / adjusted_std)
